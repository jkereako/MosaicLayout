...
extension MosaicLayout {
    ...
    private func insertCellAtIndexPath(indexPath: NSIndexPath) -> Bool {
        ...
        return !traverseOpenCells({[unowned self] cellOrigin in

            let didTraverseAllCells = self.traverseCellsForPosition(
                cellOrigin,
                withSize: aCellSize,
                closure: {[unowned self] position in
            ...
            self.traverseCellsForPosition(
                cellOrigin,
                withSize: aCellSize,
                closure: {[unowned self] aPosition in
...
//MARK:- Cell traversal
extension MosaicLayout {
    private func traverseOpenCells(closure: (CGPoint) -> Bool) -> Bool {
    ...
    private func traverseCellsForPosition(position: CGPoint, withSize size: CGSize, closure: (CGPoint) -> Bool) -> Bool {
        // O(n^2)
